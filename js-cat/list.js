  ...
  dataPage.map((pageNumber, i) => {
    if (pageNumber === dotts) {
      ...
      if (currentPage == 1 && pageNumber == 1) {
        listPageHtml += `<li class="${
          pageNumber === currentPage ? "active" : ""
        }">${pageNumber}</li>`;
      } else {
        listPageHtml += `<li class="${
          pageNumber === currentPage ? "active" : ""
        }" onclick="showPage(${pageNumber})">${pageNumber}</li>`;
      }
    }
  });
...
function showData(list, page) {
  ...
  if (page > 1) {
    document.querySelector("#game-content").className =
      "grid grid-cols-1 gap-[1.875rem] md:grid-cols-3 lg:grid-cols-5";
  } else {
    document.querySelector("#game-content").className =
      "p-4 grid-container gap-4 grid-flow-row-dense";
  }
...
let listGame;
let t = Date.now();
fetch(`/data-json/search.json?v=${t}`)
  .then((res) => res.json())
